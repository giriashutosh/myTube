    ...
    return (
        <div className='grid grid-flow-col flex items-center p-2 shadow-lg'>
            ...
            <div className='grid-col-10'>
                <div className='flex'>
                    <input className='w-[29rem] pl-4 border-2 rounded-l-full h-10 outline-none start-2' value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}></input>
                    <button className='bg-gray-100 flex items-center p-2 rounded-r-full'>🔍</button>
                </div>

                {suggestions.length === 0 ? (<div></div>) : (<div className='fixed border border-gray shadow-lg bg-white rounded-lg w-[29rem] py-2 px-2 '>
                    <ul>
                        {
